...
### Latest Deliverable (v3.7.8-dev)
*   **Wind Speed Calibration (US003)**:
    *   **High-Precision Formula**: Refactored the Arduino firmware to use a linear calibration curve ($v = f \cdot 0.097 + 0.05$) tailored for custom 4-cup carbon-fiber hardware.
    *   **Overflow Protection**: Upgraded pulse counting logic to use `unsigned int`, enabling reliable measurement during high-velocity wind events without counter rollover.
    *   **Atomic Pulse Capture**: Implemented interrupt-safe data reading to ensure thread-safe communication between the sensor ISR and the telemetry transmission loop.
    *   **Firmware Consolidation**: Standardized on `weatherstation.ino` v2.1 and removed experimental/deprecated codebase versions.

*   **Initial Point Alignment & Precision (US004)**:
...
## Bug Fixes & Refinements (v3.7.8-dev)
*   **Resolved Counter Overflows**: Fixed a critical bug where wind speeds above ~24 m/s caused the Arduino counter to reset due to 8-bit integer limitations.
*   **Improved Low-Wind Accuracy**: Introduced a minimal friction offset and frequency threshold to provide realistic readings in near-calm conditions.
*   **Codebase Hygiene**: Purged experimental Hungrian-language firmware versions to maintain project architectural standards.
